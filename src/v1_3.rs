/*!
unofficial [keyauth](https://keyauth.cc) library that uses 1.3 api version
...
impl KeyauthApi {
    ...
    /// Optional network hardening: allowlist hosts (exact or *.domain)
    ...
    pub fn clear_allowed_hosts(&mut self) {
...
{}"#,
    ...
    fn ensure_allowed_host(&self, url: &str) -> Result<(), String> {
        let parsed = Url::parse(url).map_err(|_| "invalid api url".to_string())?;
        ...
        for entry in &self.allowed_hosts {
            ...
            if let Some(suffix) = entry_l.strip_prefix("*.") {
                if host == suffix || host.ends_with(&format!(".{}", suffix)) {
                    return Ok(());
                }
            } else if host == entry_l {
                return Ok(());
            }
        }
